...
tiup cluster start dmmtest
   + Confirme the mysql connection 
      mysql -h 34.84.33.82 -u root -P 4000
      select tidb_version()
   + Open the dashboard
   /etc/tidb/setDashboard.sh
   http://35.189.141.234:2379/dashboard
   + Open the grafana
   http://34.146.245.239:3000

** Restore the data back to data
gcloud init

tiup bench tpcc --warehouses 100 --parts 4 prepare -H 10.146.15.210
tiup bench tpcc --warehouses 1000 --parts 4 prepare -T 16 -H 10.146.0.53
